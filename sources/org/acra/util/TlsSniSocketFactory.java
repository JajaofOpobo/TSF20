...
public class TlsSniSocketFactory implements LayeredSocketFactory {
    ...
    private void establishAndVerify(SSLSocket sSLSocket, String str) {
        ...
        ACRA.log.mo7i(TAG, "Established " + session.getProtocol() + " connection with " + session.getPeerHost() + " using " + session.getCipherSuite());
    }
    ...
    private void setTlsParameters(SSLSocket sSLSocket) {
        ...
        ACRA.log.mo5v(TAG, "Setting allowed TLS protocols: " + TextUtils.join(", ", linkedList));
        ...
        if (Compatibility.getAPILevel() < 21) {
            ...
            ACRA.log.mo5v(TAG, "Setting allowed TLS ciphers: " + TextUtils.join(", ", hashSet));
    ...
    private void setSniHostname(SSLSocket sSLSocket, String str) {
        if (Compatibility.getAPILevel() >= 17) {
            ACRA.log.mo11d(TAG, "Using documented SNI with host name " + str);
            this.sslSocketFactory.setHostname(sSLSocket, str);
            return;
        }
        ACRA.log.mo11d(TAG, "No documented SNI support on Android <4.2, trying reflection method with host name " + str);
        try {
            sSLSocket.getClass().getMethod("setHostname", String.class).invoke(sSLSocket, str);
        } catch (Exception e) {
            ACRA.log.mo2w(TAG, "SNI not usable", e);
        }
    }
}
