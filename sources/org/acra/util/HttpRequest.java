...
public final class HttpRequest {
            ...
            if (iOException instanceof SocketTimeoutException) {
                if (i <= this.maxNrRetries) {
                    if (this.httpParams != null) {
                        ...
                        ACRA.log.mo11d(ACRA.LOG_TAG, "SocketTimeOut - increasing time out to " + soTimeout + " millis and trying again");
                    } else {
                        ACRA.log.mo11d(ACRA.LOG_TAG, "SocketTimeOut - no HttpParams, cannot increase time out. Trying again with current settings");
                    }
                    return true;
                }
                ACRA.log.mo11d(ACRA.LOG_TAG, "SocketTimeOut but exceeded max number of retries : " + this.maxNrRetries);
            }
            return false;
        }
    }
    ...
    public void send(Context context, URL url, HttpSender.Method method, String str, HttpSender.Type type) {
        HttpClient httpClient = getHttpClient(context);
        HttpEntityEnclosingRequestBase httpRequest = getHttpRequest(url, method, str, type);
        ACRA.log.mo11d(ACRA.LOG_TAG, "Sending request to " + url);
