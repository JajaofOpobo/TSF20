...
public final class ConfigurationCollector {
        ...
        for (Field field : Configuration.class.getFields()) {
            if (Modifier.isStatic(field.getModifiers()) && Modifier.isFinal(field.getModifiers())) {
                ...
                try {
                    ...
                    ACRA.log.mo2w(ACRA.LOG_TAG, "Error while inspecting device configuration: ", e);
                } catch (IllegalArgumentException e2) {
                    ACRA.log.mo2w(ACRA.LOG_TAG, "Error while inspecting device configuration: ", e2);
                }
            }
        }
    ...
    private String toString(Configuration configuration) {
        ...
        for (Field field : configuration.getClass().getFields()) {
            try {
                ...
                ACRA.log.mo8e(ACRA.LOG_TAG, "Error while inspecting device configuration: ", e);
            } catch (IllegalArgumentException e2) {
                ACRA.log.mo8e(ACRA.LOG_TAG, "Error while inspecting device configuration: ", e2);
            }
        }
        return sb.toString();
    }
    ...
    public static String collectConfiguration(Context context) {
        try {
            ...
            ACRA.log.mo2w(ACRA.LOG_TAG, "Couldn't retrieve CrashConfiguration for : " + context.getPackageName(), e);
