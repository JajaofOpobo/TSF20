    ...
    public static String collectDisplays(Context context) {
        ...
        if (Compatibility.getAPILevel() < 17) {
            ...
            try {
                ...
                ACRA.log.mo2w(ACRA.LOG_TAG, "Error while collecting DisplayManager data: ", e);
                displayArr = null;
            } catch (IllegalArgumentException e2) {
                ACRA.log.mo2w(ACRA.LOG_TAG, "Error while collecting DisplayManager data: ", e2);
                displayArr = null;
            } catch (NoSuchFieldException e3) {
                ACRA.log.mo2w(ACRA.LOG_TAG, "Error while collecting DisplayManager data: ", e3);
                displayArr = null;
            } catch (NoSuchMethodException e4) {
                ACRA.log.mo2w(ACRA.LOG_TAG, "Error while collecting DisplayManager data: ", e4);
                displayArr = null;
            } catch (SecurityException e5) {
                ACRA.log.mo2w(ACRA.LOG_TAG, "Error while collecting DisplayManager data: ", e5);
                displayArr = null;
            } catch (InvocationTargetException e6) {
                ACRA.log.mo2w(ACRA.LOG_TAG, "Error while collecting DisplayManager data: ", e6);
                displayArr = null;
            }
        }
