...
public final class SendWorker extends Thread {
    ...
    private void approvePendingReports() {
        ...
        ACRA.log.mo11d(ACRA.LOG_TAG, "Mark all pending reports as approved.");
        for (String str : new CrashReportFinder(this.context).getCrashReportFiles()) {
            if (!this.fileNameParser.isApproved(str)) {
                ...
                if (!file.renameTo(file2)) {
                    ACRA.log.mo9e(ACRA.LOG_TAG, "Could not rename approved report from " + file + " to " + file2);
                }
            }
        }
    }
    ...
    private void checkAndSendReports(Context context, boolean z) {
        int i = 0;
        ACRA.log.mo11d(ACRA.LOG_TAG, "#checkAndSendReports - start");
        ...
        for (String str : crashReportFiles) {
            if (!z || this.fileNameParser.isSilent(str)) {
                ...
                ACRA.log.mo7i(ACRA.LOG_TAG, "Sending file " + str);
                try {
                    sendCrashReport(new CrashReportPersister(context).load(str));
                    deleteFile(context, str);
                } catch (IOException e) {
                    ACRA.log.mo8e(ACRA.LOG_TAG, "Failed to load crash report for " + str, e);
                    deleteFile(context, str);
                } catch (RuntimeException e2) {
                    ACRA.log.mo8e(ACRA.LOG_TAG, "Failed to send crash reports for " + str, e2);
                    deleteFile(context, str);
                } catch (ReportSenderException e3) {
                    ACRA.log.mo8e(ACRA.LOG_TAG, "Failed to send crash report for " + str, e3);
                }
                i++;
            }
        }
        ACRA.log.mo11d(ACRA.LOG_TAG, "#checkAndSendReports - finish");
    }
        ...
        if (!ACRA.isDebuggable() || ACRA.getConfig().sendReportsInDevMode()) {
            ...
            for (ReportSender reportSender : this.reportSenders) {
                try {
                    ...
                    ACRA.log.mo3w(ACRA.LOG_TAG, "ReportSender of class " + reportSender.getClass().getName() + " failed but other senders completed their task. ACRA will not send this report again.");
                }
            }
        }
    }
    ...
    private void deleteFile(Context context, String str) {
        if (!context.deleteFile(str)) {
            ACRA.log.mo3w(ACRA.LOG_TAG, "Could not delete error report : " + str);
        }
    }
}
