...
public class ErrorReporter implements Thread.UncaughtExceptionHandler {
        ...
        try {
            if (!this.enabled) {
                if (this.mDfltExceptionHandler != null) {
                    ACRA.log.mo9e(ACRA.LOG_TAG, "ACRA is disabled for " + this.mContext.getPackageName() + " - forwarding uncaught Exception on to default ExceptionHandler");
                    this.mDfltExceptionHandler.uncaughtException(thread, th);
                } else {
                    ACRA.log.mo9e(ACRA.LOG_TAG, "ACRA is disabled for " + this.mContext.getPackageName() + " - no default ExceptionHandler");
                    ACRA.log.mo8e(ACRA.LOG_TAG, "ACRA caught a " + th.getClass().getSimpleName() + " for " + this.mContext.getPackageName(), th);
                }
            } else {
                ACRA.log.mo8e(ACRA.LOG_TAG, "ACRA caught a " + th.getClass().getSimpleName() + " for " + this.mContext.getPackageName(), th);
                ACRA.log.mo11d(ACRA.LOG_TAG, "Building report");
    ...
    public void endApplication(Thread thread, Throwable th) {
        ...
        if ((thread != null) && z && this.mDfltExceptionHandler != null) {
            ACRA.log.mo11d(ACRA.LOG_TAG, "Handing Exception on to default ExceptionHandler");
            this.mDfltExceptionHandler.uncaughtException(thread, th);
            return;
        }
        ACRA.log.mo8e(ACRA.LOG_TAG, String.valueOf(this.mContext.getPackageName()) + " fatal error : " + th.getMessage(), th);
        Activity activity = this.lastActivityCreated.get();
        if (activity != null) {
            ACRA.log.mo7i(ACRA.LOG_TAG, "Finishing the last Activity prior to killing the Process");
            activity.finish();
            ACRA.log.mo7i(ACRA.LOG_TAG, "Finished " + activity.getClass());
    ...
    public void handleSilentException(Throwable th) {
        if (this.enabled) {
            ...
            ACRA.log.mo11d(ACRA.LOG_TAG, "ACRA sent Silent report.");
            return;
        }
        ACRA.log.mo11d(ACRA.LOG_TAG, "ACRA is disabled. Silent report not sent.");
    }

    public void setEnabled(boolean z) {
        ACRA.log.mo7i(ACRA.LOG_TAG, "ACRA is " + (z ? "enabled" : "disabled") + " for " + this.mContext.getPackageName());
        this.enabled = z;
    }
    ...
    public void checkReportsOnApplicationStart() {
        ...
        if (crashReportFiles != null && crashReportFiles.length > 0) {
            ...
            ACRA.log.mo5v(ACRA.LOG_TAG, "About to start ReportSenderWorker from #checkReportOnApplicationStart");
    ...
    public void report(final ReportBuilder reportBuilder) {
        ...
        if (this.enabled) {
            try {
                ...
                ACRA.log.mo11d(ACRA.LOG_TAG, "Failed to initlize " + this.exceptionHandlerInitializer + " from #handleException");
            }
            ...
            if (reportingInteractionMode == ReportingInteractionMode.SILENT || reportingInteractionMode == ReportingInteractionMode.TOAST || this.prefs.getBoolean(ACRA.PREF_ALWAYS_ACCEPT, false)) {
                ACRA.log.mo11d(ACRA.LOG_TAG, "About to start ReportSenderWorker from #handleException");
                ...
                if (reportingInteractionMode == ReportingInteractionMode.NOTIFICATION) {
                    ACRA.log.mo11d(ACRA.LOG_TAG, "Creating Notification.");
            ...
            if (z2) {
                ...
                new Thread() { // from class: org.acra.ErrorReporter.4
                    @Override // java.lang.Thread, java.lang.Runnable
                    public void run() {
                        ACRA.log.mo11d(ACRA.LOG_TAG, "Waiting for 2000 millis from " + timeHelper.initialTimeMillis + " currentMillis=" + System.currentTimeMillis());
                        while (timeHelper.getElapsedTime() < 2000) {
                            try {
                                Thread.sleep(100L);
                            } catch (InterruptedException e2) {
                                ACRA.log.mo10d(ACRA.LOG_TAG, "Interrupted while waiting for Toast to end.", e2);
                            }
                        }
            ...
            new Thread() { // from class: org.acra.ErrorReporter.5
                @Override // java.lang.Thread, java.lang.Runnable
                public void run() {
                    ACRA.log.mo11d(ACRA.LOG_TAG, "Waiting for Toast");
                    while (!ErrorReporter.this.toastWaitEnded) {
                        try {
                            Thread.sleep(100L);
                        } catch (InterruptedException e2) {
                            ACRA.log.mo10d(ACRA.LOG_TAG, "Error : ", e2);
                        }
                    }
                    ACRA.log.mo11d(ACRA.LOG_TAG, "Finished waiting for Toast");
                    if (startSendingReports != null) {
                        ACRA.log.mo11d(ACRA.LOG_TAG, "Waiting for Worker");
                        while (startSendingReports.isAlive()) {
                            try {
                                Thread.sleep(100L);
                            } catch (InterruptedException e3) {
                                ACRA.log.mo10d(ACRA.LOG_TAG, "Error : ", e3);
                            }
                        }
                        ACRA.log.mo11d(ACRA.LOG_TAG, "Finished waiting for Worker");
                    }
                    if (z3) {
                        ACRA.log.mo11d(ACRA.LOG_TAG, "Creating CrashReportDialog for " + reportFileName);
                    ...
                    ACRA.log.mo11d(ACRA.LOG_TAG, "Wait for Toast + worker ended. Kill Application ? " + reportBuilder.mEndsApplication);
    ...
    public Intent createCrashReportDialogIntent(String str, ReportBuilder reportBuilder) {
        ACRA.log.mo11d(ACRA.LOG_TAG, "Creating DialogIntent for " + str + " exception=" + reportBuilder.mException);
    ...
    private void createNotification(String str, ReportBuilder reportBuilder) {
        ...
        ACRA.log.mo11d(ACRA.LOG_TAG, "Creating Notification for " + str);
    ...
    private void saveCrashReportFile(String str, CrashReportData crashReportData) {
        try {
            ACRA.log.mo11d(ACRA.LOG_TAG, "Writing crash report file " + str + ".");
            new CrashReportPersister(this.mContext).store(crashReportData, str);
        } catch (Exception e) {
            ACRA.log.mo8e(ACRA.LOG_TAG, "An error occurred while writing the report file...", e);
        }
    }
    ...
    private void deletePendingReports(boolean z, boolean z2, int i) {
        ...
        for (int i2 = 0; i2 < crashReportFiles.length - i; i2++) {
            ...
            if ((isApproved && z) || (!isApproved && z2)) {
                ...
                ACRA.log.mo11d(ACRA.LOG_TAG, "Deleting file " + str);
                if (!file.delete()) {
                    ACRA.log.mo9e(ACRA.LOG_TAG, "Could not delete report : " + file);
                }
            }
        }
    }
    ...
    public void setDefaultReportSenders() {
        ...
        if (!"".equals(config.mailTo())) {
            ACRA.log.mo3w(ACRA.LOG_TAG, String.valueOf(application.getPackageName()) + " reports will be sent by email (if accepted by user).");
