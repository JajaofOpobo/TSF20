...
public class World {
    ...
    private float reportRayFixture(long j, float f, float f2, float f3, float f4, float f5) {
        if (this.rayCastCallback != null) {
            return this.rayCastCallback.reportRayFixture((Fixture) this.fixtures.get(j), this.rayPoint.set(f, f2), this.rayNormal.set(f3, f4), f5);
        }
        return 0.0f;
    }
    ...
    public Body createBody(BodyDef bodyDef) {
        ...
        String str = "createBody id:" + body.addr;
        return body;
    }

    public void destroyAllBody() {
        LongMap.Entries entries = this.bodies.entries();
        while (entries.hasNext()) {
            Body body = (Body) entries.next().value;
            for (int i = 0; i < body.getFixtureList().size(); i++) {
                this.fixtures.remove(((Fixture) body.getFixtureList().get(i)).addr);
            }
            for (int i2 = 0; i2 < body.getJointList().size(); i2++) {
                this.joints.remove(((JointEdge) body.getJointList().get(i2)).joint.addr);
            }
            String str = "destroyBody id:" + body.addr;
    ...
    public Joint createJoint(JointDef jointDef) {
        ...
        String str = "createJoint id:" + joint.addr;
        return joint;
    }
    ...
    public void destroyAllJoint() {
        LongMap.Entries entries = this.joints.entries();
        while (entries.hasNext()) {
            Joint joint = (Joint) entries.next().value;
            joint.jointEdgeA.other.joints.remove(joint.jointEdgeB);
            joint.jointEdgeB.other.joints.remove(joint.jointEdgeA);
            String str = "destroyJoint id:" + joint.addr;
    ...
    public List getContactList() {
        ...
        for (int i2 = 0; i2 < contactCount; i2++) {
            Contact contact = (Contact) this.freeContacts.get(i2);
    ...
    private boolean contactFilter(long j, long j2) {
        ...
        Filter filterData = ((Fixture) this.fixtures.get(j)).getFilterData();
        Filter filterData2 = ((Fixture) this.fixtures.get(j2)).getFilterData();
