package com.tsf.shell.e.g.a;

import android.view.MotionEvent;
import com.censivn.C3DEngine.b.f.j;
import com.censivn.C3DEngine.b.f.l;
import com.censivn.C3DEngine.b.f.o;
import com.censivn.C3DEngine.b.h.b.i;
import com.tsf.shell.R;
import com.tsf.shell.utils.w;

/* loaded from: classes.dex */
public class f extends a {
    public b a;
    private i b;
    private com.tsf.shell.manager.p.g c;
    private o d;

    public f(b bVar) {
        super(R.string.text_mark);
        this.a = bVar;
        this.d = new o();
        this.d.c(R.string.notic_mark_introduction);
        this.d.d(34);
        this.d.a(true);
        com.tsf.shell.manager.o.c.a(this.d, com.tsf.shell.manager.o.c.n);
        setMenuHeight(com.censivn.C3DEngine.b.b.a.a(430.0f));
        this.b = new i(com.censivn.C3DEngine.b.b.a.D, this.a.getHeight(), com.censivn.C3DEngine.b.b.a.c * 20.0f, com.censivn.C3DEngine.b.b.a.c * 20.0f, com.tsf.shell.manager.o.b.c.T * 0.9f, com.tsf.shell.manager.o.b.c.U * com.censivn.C3DEngine.b.b.a.c * 0.9f);
        this.b.a(new com.censivn.C3DEngine.b.h.b.b() { // from class: com.tsf.shell.e.g.a.f.1
            @Override // com.censivn.C3DEngine.b.h.b.b
            public int a() {
                return com.tsf.shell.manager.a.v.f.i();
            }

            @Override // com.censivn.C3DEngine.b.h.b.b
            public j a(int i, j jVar) {
                com.tsf.shell.e.d.b.a.e eVar = (com.tsf.shell.e.d.b.a.e) com.tsf.shell.manager.a.v.f.a(i);
                com.tsf.shell.e.d.b.a.c e = eVar.e();
                e.removeFromParent();
                if (eVar.i()) {
                    e.g();
                } else {
                    e.h();
                }
                if (eVar.n()) {
                    e.c();
                } else {
                    e.d();
                }
                return e;
            }
        });
        this.d.position().y = com.censivn.C3DEngine.b.b.a.a(300.0f);
        this.b.a().addChild(this.d);
        this.b.a(new com.censivn.C3DEngine.b.h.b.g() { // from class: com.tsf.shell.e.g.a.f.2
            @Override // com.censivn.C3DEngine.b.h.b.g
            public void a(j jVar, int i, MotionEvent motionEvent) {
                com.tsf.shell.e.d.b.a.e eVar = (com.tsf.shell.e.d.b.a.e) com.tsf.shell.manager.a.v.f.a(i);
                w.b();
                com.tsf.shell.manager.a.v.f.a((com.tsf.shell.e.f.f) eVar);
                f.this.b.d();
            }
        });
        this.c = com.tsf.shell.manager.a.a.a(this, com.tsf.shell.manager.p.f.h);
    }

    @Override // com.tsf.shell.e.e.g.a.e
    public l getContentContainer() {
        return this.b;
    }

    @Override // com.tsf.shell.e.e.g.a.e
    public void onLayout(float f, float f2, float f3, float f4) {
        this.b.a(f, f2, f3, f4);
        this.d.f((int) (com.censivn.C3DEngine.b.b.a.D - com.censivn.C3DEngine.b.b.a.a(150.0f)));
    }

    @Override // com.tsf.shell.e.e.g.a.e
    public void onShow() {
        this.b.d();
        this.a.a();
    }

    @Override // com.tsf.shell.e.g.a.a
    public float b() {
        return com.censivn.C3DEngine.b.b.a.a(120.0f);
    }

    @Override // com.tsf.shell.e.g.a.a
    public void c() {
    }

    @Override // com.tsf.shell.e.e.g.a.e
    public void onHide() {
        super.onHide();
        this.c.b();
        int i = com.tsf.shell.manager.a.v.f.i();
        for (int i2 = 0; i2 < i; i2++) {
            ((com.tsf.shell.e.d.b.a.e) com.tsf.shell.manager.a.v.f.a(i2)).l();
        }
    }

    @Override // com.tsf.shell.e.e.g.a.e
    public void onRecycle() {
        super.onRecycle();
        this.d.a();
        int i = com.tsf.shell.manager.a.v.f.i();
        for (int i2 = 0; i2 < i; i2++) {
            com.tsf.shell.e.d.b.a.e eVar = (com.tsf.shell.e.d.b.a.e) com.tsf.shell.manager.a.v.f.a(i2);
            eVar.h();
            eVar.d();
            eVar.a();
        }
        com.tsf.shell.manager.a.v.f.j();
    }
}
