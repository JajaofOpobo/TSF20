package com.tsf.shell.widget.a;

import android.graphics.Bitmap;
import android.graphics.Canvas;
import android.graphics.Color;
import android.graphics.Paint;
import android.graphics.PaintFlagsDrawFilter;
import android.text.format.DateFormat;
import com.censivn.C3DEngine.api.core.VTextureManager;
import com.censivn.C3DEngine.api.element.TextureElement;
import com.censivn.C3DEngine.api.shell.VInformation;
import com.censivn.C3DEngine.b.g.w;
import com.censivn.C3DEngine.b.g.x;
import java.util.ArrayList;
import java.util.Calendar;
import java.util.Iterator;

/* loaded from: classes.dex */
public class a extends com.censivn.C3DEngine.b.f.l {
    private TextureElement A;
    private TextureElement B;
    private int D;
    private int E;
    private int F;
    private int G;
    private int H;
    private int I;
    private com.censivn.C3DEngine.b.f.a.a J;
    int a;
    public d c;
    private VTextureManager d;
    private long f;
    private int g;
    private int h;
    private b i;
    private com.censivn.C3DEngine.b.f.a.a j;
    private e k;
    private e l;
    private k m;
    private k n;
    private h o;
    private h p;
    private int x;
    private TextureElement y;
    private TextureElement z;
    private final int e = 60;
    public boolean b = false;
    private long q = 0;
    private long r = 0;
    private long s = 0;
    private ArrayList t = new ArrayList();
    private c[] u = new c[60];
    private int v = -1;
    private int w = this.v;
    private Object C = new Object();

    public a(VTextureManager vTextureManager, d dVar) {
        this.d = vTextureManager;
        this.c = dVar;
        a();
    }

    public void a() {
        float XHDScale = VInformation.XHDScale();
        Bitmap a = a(this.v, (int) (30.0f * XHDScale));
        this.B = this.d.createTexture(a, true);
        a.recycle();
        this.j = new com.censivn.C3DEngine.b.f.a.a(60, 20, 20, 20, null, true);
        this.j.textures().addElement(this.B);
        for (int i = 0; i < 60; i++) {
            com.censivn.C3DEngine.b.f.a.b a2 = this.j.a(i);
            this.u[i] = new c(a2, 250.0f, ((-6.0f) * i) + 90.0f);
            a2.b(0, 0, 1, 1);
            a2.h();
        }
        for (int i2 = 0; i2 < 60; i2 += 2) {
            com.censivn.C3DEngine.b.f.a.b a3 = this.j.a(i2);
            int i3 = i2 + 3;
            if (i3 >= 60) {
                i3 %= 60;
            }
            g.a("index:" + i2 + "  next:" + i3);
            this.t.add(new l(a3, this.j.a(i3)));
        }
        this.j.calAABB();
        this.x = (int) a(this.j.a(0), this.j.a(3));
        Bitmap a4 = a(this.v, this.x, 4);
        this.y = this.d.createTexture(a4, true);
        a4.recycle();
        int size = this.t.size();
        this.J = new com.censivn.C3DEngine.b.f.a.a(size, 1, this.x, 4, null, true);
        this.J.textures().addElement(this.y);
        for (int i4 = 0; i4 < size; i4++) {
            l lVar = (l) this.t.get(i4);
            com.censivn.C3DEngine.b.f.a.b a5 = this.J.a(i4);
            a5.a(0.0f);
            a5.b(0, 0, this.x, 4);
            lVar.a(a5);
        }
        Bitmap b = b(this.v, (int) (12.0f * XHDScale), (int) (230.0f * XHDScale));
        this.z = this.d.createTexture(b, true);
        b.recycle();
        this.m = new k(12.0f, 230.0f);
        this.m.textures().addElement(this.z);
        this.m.moveAllPointsPX(0.0f, 95.0f, 0.0f);
        Bitmap b2 = b(this.v, (int) (16.0f * XHDScale), (int) (170.0f * XHDScale));
        this.A = this.d.createTexture(b2, true);
        b2.recycle();
        this.n = new k(16.0f, 170.0f);
        this.n.textures().addElement(this.A);
        this.n.moveAllPointsPX(0.0f, 65.0f, 0.0f);
        this.n.rotation().z = 30.0f;
        this.o = new h(this.d, 60, "  ");
        this.k = new e(60.0f, 60.0f);
        this.k.position().y = 300.0f;
        this.k.textures().addElement(this.o.a);
        this.p = new h(this.d, 50, "  ");
        this.l = new e(50.0f, 50.0f);
        this.l.position().y = 300.0f;
        this.l.textures().addElement(this.p.a);
        scale().x = XHDScale;
        scale().y = XHDScale;
        scale().z = XHDScale;
        addChild(this.J);
        addChild(this.j);
        addChild(this.m);
        addChild(this.n);
        addChild(this.k);
        addChild(this.l);
        calAABB();
        setAABBPX((-300.0f) * XHDScale, (-300.0f) * XHDScale, 0.0f, 300.0f * XHDScale, 300.0f * XHDScale, 0.0f);
        this.i = new b(this);
        this.i.start();
        b(this.c.b);
        a(this.c.a);
    }

    public void b() {
        this.b = true;
        w.b(this.k);
        w.b(this.l);
    }

    public void c() {
        this.b = false;
        w.c(this.k);
        w.c(this.l);
        synchronized (this.C) {
            this.C.notifyAll();
        }
    }

    public void d() {
        if (this.i != null) {
            this.i.a();
        }
        com.censivn.C3DEngine.a.g().a(this.y);
        com.censivn.C3DEngine.a.g().a(this.z);
        com.censivn.C3DEngine.a.g().a(this.A);
        com.censivn.C3DEngine.a.g().a(this.B);
        this.o.a();
        this.p.a();
    }

    @Override // com.censivn.C3DEngine.b.f.j
    public void onDrawStart() {
        this.q++;
        Iterator it = this.t.iterator();
        while (it.hasNext()) {
            ((l) it.next()).a(this.q);
        }
        if (!this.b) {
            this.f = System.currentTimeMillis();
            int i = (int) ((this.f / 1000) % 60);
            if (i != this.a) {
                this.a = i;
                i();
            }
            int i2 = (int) ((this.f / 1000) / 60);
            if (i2 != this.g) {
                this.g = i2;
                g();
                int i3 = i2 / 60;
                if (i3 != this.h) {
                    this.h = i3;
                }
            }
        }
        f();
    }

    private Bitmap a(int i, int i2) {
        Bitmap createBitmap = Bitmap.createBitmap(i2, i2, Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(createBitmap);
        canvas.setDrawFilter(new PaintFlagsDrawFilter(0, 3));
        Paint paint = new Paint();
        paint.setAntiAlias(true);
        paint.setColor(i);
        canvas.drawCircle(i2 / 2.0f, i2 / 2.0f, (i2 / 2.0f) - 2.0f, paint);
        return createBitmap;
    }

    private Bitmap a(int i, int i2, int i3) {
        Bitmap createBitmap = Bitmap.createBitmap(i2, i3, Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(createBitmap);
        canvas.setDrawFilter(new PaintFlagsDrawFilter(0, 3));
        Paint paint = new Paint();
        paint.setAntiAlias(true);
        paint.setColor(i);
        canvas.drawRect(1.0f, 1.0f, i2 - 1, i3 - 1, paint);
        return createBitmap;
    }

    private Bitmap b(int i, int i2, int i3) {
        Bitmap createBitmap = Bitmap.createBitmap(i2, i3, Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(createBitmap);
        canvas.setDrawFilter(new PaintFlagsDrawFilter(0, 3));
        Paint paint = new Paint();
        paint.setAntiAlias(true);
        paint.setColor(i);
        int i4 = i2 - 2;
        canvas.drawRect(1.0f, i4 / 2, i2 - 1, (i3 - 1) - (i4 / 2), paint);
        canvas.drawCircle(i2 / 2.0f, i2 / 2.0f, i4 / 2.0f, paint);
        canvas.drawCircle(i2 / 2.0f, i3 - (i4 / 2), i4 / 2.0f, paint);
        return createBitmap;
    }

    private float a(com.censivn.C3DEngine.b.f.a.b bVar, com.censivn.C3DEngine.b.f.a.b bVar2) {
        float f = bVar2.i().x - bVar.i().x;
        float f2 = bVar2.i().y - bVar.i().y;
        return (float) Math.sqrt((f * f) + (f2 * f2));
    }

    public void e() {
    }

    public void a(boolean z) {
        if (z) {
            this.k.visible(true);
            this.l.visible(true);
        } else {
            this.k.visible(false);
            this.l.visible(false);
        }
    }

    private void b(int i) {
        this.w = i;
        this.v = i;
        int red = Color.red(this.w);
        this.D = red;
        this.G = red;
        int green = Color.green(this.w);
        this.E = green;
        this.H = green;
        int blue = Color.blue(this.w);
        this.F = blue;
        this.I = blue;
        c(Color.argb(255, this.G, this.H, this.I));
    }

    public void a(int i) {
        this.w = i;
        this.D = Color.red(this.w);
        this.E = Color.green(this.w);
        this.F = Color.blue(this.w);
    }

    private void f() {
        float abs = Math.abs(this.D - this.G);
        float abs2 = Math.abs(this.E - this.H);
        float abs3 = Math.abs(this.F - this.I);
        if (abs > 1.0f || abs2 > 1.0f || abs3 > 1.0f) {
            invalidate();
            this.G = (int) (((this.D - this.G) * 0.1f) + this.G);
            this.H = (int) (((this.E - this.H) * 0.1f) + this.H);
            this.I = (int) ((0.1f * (this.F - this.I)) + this.I);
            c(Color.argb(255, this.G, this.H, this.I));
            return;
        }
        if (this.v != this.w) {
            this.G = this.D;
            this.H = this.E;
            this.I = this.F;
            this.v = this.w;
            g.a("Widget Cube Slient First");
            c(Color.argb(255, this.G, this.H, this.I));
        }
    }

    private void c(int i) {
        this.v = i;
        for (int i2 = 0; i2 < 60; i2++) {
            com.censivn.C3DEngine.b.f.a.b a = this.j.a(i2);
            a.H.set(this.v);
            a.h();
        }
        int b = this.J.b();
        for (int i3 = 0; i3 < b; i3++) {
            com.censivn.C3DEngine.b.f.a.b a2 = this.J.a(i3);
            a2.H.set(this.v);
            a2.h();
        }
        this.n.a(this.v);
        this.m.a(this.v);
        this.k.a(this.v);
        this.l.a(this.v);
    }

    private void g() {
        Calendar calendar = Calendar.getInstance();
        calendar.setTimeInMillis(System.currentTimeMillis());
        CharSequence format = DateFormat.format("h", calendar);
        int parseInt = Integer.parseInt(format.toString());
        CharSequence format2 = DateFormat.format("mm", calendar);
        int parseInt2 = Integer.parseInt(format2.toString());
        this.o.a(format.toString());
        this.p.a(format2.toString());
        float f = (((parseInt2 / 60.0f) * 360.0f) / 12.0f) + ((parseInt / 12.0f) * 360.0f);
        x xVar = new x() { // from class: com.tsf.shell.widget.a.a.1
            @Override // com.censivn.C3DEngine.b.g.x
            public void a(float f2) {
                a.this.k.c(a.this.n.rotation().z + 90.0f);
            }
        };
        xVar.e(-f);
        xVar.a(com.censivn.C3DEngine.b.g.a.g);
        w.a(this.n);
        w.a(this.n, 1500, xVar);
        float f2 = (parseInt2 / 60.0f) * 360.0f;
        float f3 = 0.0f;
        if (a(f2, f) < 6.0f) {
            f3 = 50.0f;
        }
        final float a = this.l.a();
        final float f4 = f3 - a;
        x xVar2 = new x() { // from class: com.tsf.shell.widget.a.a.2
            @Override // com.censivn.C3DEngine.b.g.x
            public void a(float f5) {
                a.this.l.c(a.this.m.rotation().z + 90.0f);
            }

            @Override // com.censivn.C3DEngine.b.g.x
            public void b(float f5) {
                a.this.l.e(a + (f4 * f5));
            }
        };
        xVar2.e(-f2);
        xVar2.a(com.censivn.C3DEngine.b.g.a.g);
        w.a(this.m);
        w.a(this.m, 1500, xVar2);
        h();
    }

    private float a(float f, float f2) {
        float abs = Math.abs(f2 - f);
        return abs > 180.0f ? 360.0f - abs : abs;
    }

    private void h() {
        if (this.q >= this.s) {
            this.r = this.q;
            this.s = this.r + 360;
            Calendar calendar = Calendar.getInstance();
            calendar.setTimeInMillis(System.currentTimeMillis());
            int parseInt = Integer.parseInt(DateFormat.format("h", calendar).toString());
            int parseInt2 = Integer.parseInt(DateFormat.format("mm", calendar).toString());
            int abs = ((int) (Math.abs(Math.toDegrees((float) Math.toRadians((((-parseInt) * 30) + 90.0f) - (((parseInt2 / 60.0f) * 360.0f) / 12.0f))) - 90.0d) / 6.0f)) * 50;
            g.a("delay:" + abs);
            x xVar = new x() { // from class: com.tsf.shell.widget.a.a.3
                @Override // com.censivn.C3DEngine.b.g.x
                public void a(float f) {
                    a.this.k.d((float) (50.0d * Math.sin(f * 3.141592653589793d)));
                }
            };
            xVar.b(abs);
            w.a(this.k);
            w.a(this.k, 1000, xVar);
            float radians = (float) Math.toRadians(90.0f - ((parseInt2 / 60.0f) * 360.0f));
            int abs2 = ((int) (Math.abs(Math.toDegrees(radians) - 90.0d) / 6.0f)) * 50;
            g.a("minRadians delay:" + Math.toDegrees(radians));
            x xVar2 = new x() { // from class: com.tsf.shell.widget.a.a.4
                @Override // com.censivn.C3DEngine.b.g.x
                public void a(float f) {
                    a.this.l.d((float) (50.0d * Math.sin(f * 3.141592653589793d)));
                }
            };
            xVar2.b(abs2);
            w.a(this.l);
            w.a(this.l, 1000, xVar2);
            int size = this.t.size();
            for (int i = 0; i < size; i++) {
                ((l) this.t.get(i)).a(i * 100, this.q);
            }
        }
    }

    private void i() {
        this.u[this.a].a();
        invalidate();
    }
}
