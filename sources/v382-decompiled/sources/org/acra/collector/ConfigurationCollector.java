...
public final class ConfigurationCollector {
    ...
    private final HashMap mValueArrays = new HashMap();
    ...
    private String getFieldValueName(Configuration configuration, Field field) {
        ...
        if (name.equals(FIELD_UIMODE)) {
            return activeFlags((SparseArray) this.mValueArrays.get(PREFIX_UI_MODE), field.getInt(configuration));
        }
        if (name.equals(FIELD_SCREENLAYOUT)) {
            return activeFlags((SparseArray) this.mValueArrays.get(PREFIX_SCREENLAYOUT), field.getInt(configuration));
        }
        SparseArray sparseArray = (SparseArray) this.mValueArrays.get(String.valueOf(name.toUpperCase()) + '_');
        if (sparseArray == null) {
            return Integer.toString(field.getInt(configuration));
        }
    ...
    private static String activeFlags(SparseArray sparseArray, int i) {
        ...
        while (true) {
            ...
            if (i4 < sparseArray.size()) {
                ...
                if (((String) sparseArray.get(keyAt)).endsWith(SUFFIX_MASK) && (i2 = i & keyAt) > 0) {
                    if (sb.length() > 0) {
                        sb.append('+');
                    }
