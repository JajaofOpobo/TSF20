...
public class ErrorReporter implements Thread.UncaughtExceptionHandler {
    ...
    ErrorReporter(Application application, SharedPreferences sharedPreferences, boolean z) {
        ...
        this.enabled = z;
        String strCollectConfiguration = ACRA.getConfig().getReportFields().contains(ReportField.INITIAL_CONFIGURATION) ? ConfigurationCollector.collectConfiguration(this.mContext) : null;
        Time time = new Time();
        time.setToNow();
    ...
    public void report(final ReportBuilder reportBuilder) {
        ...
        boolean z;
        final SendWorker sendWorkerStartSendingReports;
        ...
        if (this.enabled) {
            ...
            if (!reportBuilder.mForceSilent) {
                ...
                if (ACRA.getConfig().mode() != ReportingInteractionMode.SILENT) {
                    reportingInteractionModeMode = reportingInteractionMode;
                    z = true;
                } else {
                    reportingInteractionModeMode = reportingInteractionMode;
                    z = false;
            ...
            boolean z2 = reportingInteractionModeMode == ReportingInteractionMode.TOAST || (ACRA.getConfig().resToastText() != 0 && (reportingInteractionModeMode == ReportingInteractionMode.NOTIFICATION || reportingInteractionModeMode == ReportingInteractionMode.DIALOG));
            final TimeHelper timeHelper2 = new TimeHelper(timeHelper);
            if (z2) {
                new Thread() { // from class: org.acra.ErrorReporter.3
                    ...
                    public void run() {
                        ...
                        timeHelper2.setInitialTimeMillis(System.currentTimeMillis());
                        Looper.loop();
                    }
                }.start();
            }
            ...
            if (reportingInteractionModeMode == ReportingInteractionMode.SILENT || reportingInteractionModeMode == ReportingInteractionMode.TOAST || this.prefs.getBoolean(ACRA.PREF_ALWAYS_ACCEPT, false)) {
                ...
                sendWorkerStartSendingReports = startSendingReports(z, true);
                if (reportingInteractionModeMode == ReportingInteractionMode.SILENT && !reportBuilder.mEndsApplication) {
                    return;
                }
            } else {
                if (reportingInteractionModeMode == ReportingInteractionMode.NOTIFICATION) {
                    ACRA.log.d(ACRA.LOG_TAG, "Creating Notification.");
                    createNotification(reportFileName, reportBuilder);
                }
                sendWorkerStartSendingReports = null;
            }
            ...
            if (z2) {
                ...
                new Thread() { // from class: org.acra.ErrorReporter.4
                    @Override // java.lang.Thread, java.lang.Runnable
                    public void run() {
                        ACRA.log.d(ACRA.LOG_TAG, "Waiting for 2000 millis from " + timeHelper2.initialTimeMillis + " currentMillis=" + System.currentTimeMillis());
                        while (timeHelper2.getElapsedTime() < 2000) {
                            try {
                                Thread.sleep(100L);
            ...
            final boolean z3 = reportingInteractionModeMode == ReportingInteractionMode.DIALOG && !this.prefs.getBoolean(ACRA.PREF_ALWAYS_ACCEPT, false);
            new Thread() { // from class: org.acra.ErrorReporter.5
                ...
                public void run() {
                    ...
                    if (sendWorkerStartSendingReports != null) {
                        ACRA.log.d(ACRA.LOG_TAG, "Waiting for Worker");
                        while (sendWorkerStartSendingReports.isAlive()) {
                            try {
                                Thread.sleep(100L);
