package com.tsf.shell.p096f.p131f.p141b.p142a;

import com.censivn.C3DEngine.p031b.p038f.C0975i;
import com.censivn.C3DEngine.p031b.p038f.p039a.C0962a;
import com.censivn.C3DEngine.p031b.p038f.p039a.C0964b;
import com.censivn.C3DEngine.p031b.p041g.C1014c;
import com.censivn.C3DEngine.p031b.p041g.C1017d;
import com.tsf.shell.p096f.p153i.C3112b;
import java.util.ArrayList;
import java.util.Iterator;
/* renamed from: com.tsf.shell.f.f.b.a.f */
/* loaded from: classes.dex */
public class C2889f extends C2885d {

    /* renamed from: b */
    private boolean f9417b;

    /* renamed from: c */
    private boolean f9418c;

    public C2889f(int i, boolean z, boolean z2, boolean z3) {
        super(i, z3);
        this.f9417b = z;
        this.f9418c = z2;
    }

    @Override // com.tsf.shell.p096f.p131f.p141b.p142a.C2885d
    /* renamed from: a */
    public void mo4359a(ArrayList<C0975i> arrayList, boolean z, Runnable runnable) {
        m4364a(arrayList, z, this.f9417b, runnable);
    }

    @Override // com.tsf.shell.p096f.p131f.p141b.p142a.C2885d
    /* renamed from: b */
    public void mo4358b(ArrayList<C0975i> arrayList, boolean z, Runnable runnable) {
        C1017d c1017d;
        C1017d c1017d2 = null;
        Iterator<C0975i> it = arrayList.iterator();
        while (it.hasNext()) {
            C0975i next = it.next();
            C1014c.m10326a(next);
            if (next instanceof C3112b) {
                C1017d c1017d3 = new C1017d();
                if (z) {
                    c1017d3.mo10301f(0.0f);
                    c1017d3.mo10298h(0.0f);
                    c1017d3.m10294l(0.0f);
                    c1017d3.m10293m(0.0f);
                    c1017d3.m10314a(0);
                    if (this.f9418c) {
                        c1017d3.m10305d(m4365a(50));
                        c1017d3.m10303e(m4365a(180));
                        c1017d3.m10307c(m4365a(50));
                    }
                    C1014c.m10325a(next, 600, c1017d3);
                    c1017d = c1017d3;
                    c1017d2 = c1017d;
                } else {
                    next.position().f2526x = 0.0f;
                    next.position().f2527y = 0.0f;
                    next.scale().f2526x = 0.0f;
                    next.scale().f2527y = 0.0f;
                    next.alpha(0.0f);
                    if (this.f9418c) {
                        next.rotation().f2526x = m4365a(50);
                        next.rotation().f2527y = m4365a(180);
                        next.rotation().f2528z = m4365a(50);
                    }
                }
            }
            c1017d = c1017d2;
            c1017d2 = c1017d;
        }
        if (c1017d2 != null) {
            c1017d2.m10311a(runnable);
        } else if (runnable != null) {
            runnable.run();
        }
    }

    @Override // com.tsf.shell.p096f.p131f.p141b.p142a.C2885d, com.tsf.shell.p096f.p107d.p116e.C2437a.InterfaceC2441a
    /* renamed from: a */
    public void mo4360a(C0962a c0962a, Runnable runnable) {
        C1017d c1017d = null;
        for (int i = 0; i < c0962a.m10459a(); i++) {
            C0964b m10456a = c0962a.m10456a(i);
            c1017d = new C1017d();
            c1017d.mo10298h(0.0f);
            c1017d.mo10301f(0.0f);
            c1017d.m10294l(0.0f);
            c1017d.m10293m(0.0f);
            if (this.f9418c) {
                c1017d.m10305d(m4365a(50));
                c1017d.m10303e(m4365a(180));
                c1017d.m10307c(m4365a(50));
            }
            C1014c.m10326a(m10456a);
            C1014c.m10325a(m10456a, 700, c1017d);
        }
        if (c1017d != null) {
            c1017d.m10311a(runnable);
            c1017d.m10308b(runnable);
        } else if (runnable != null) {
            runnable.run();
        }
    }
}
