package com.tsf.shell.f._d.d;

import com.badlogic.gdx.math.Vector2;
import com.badlogic.gdx.physics.box2d.World;
import com.tsf.shell.f.f.WorkspacePage;

/* JADX INFO: loaded from: C:\Users\Jaja\AndroidStudioProjects\TSF20\resources-Prime\classes.dex */
public class DashAnimationConfig {
    public static float a = 30.0f;
    private boolean d = false;
    private Vector2 c = new Vector2(0.0f, 0.0f);
    public World b;

    public boolean a() {
        return this.d;
    }

    public void a(boolean z) {
        this.d = z;
    }

    public void a(g gVar) {
        com.tsf.shell.manager.app.v.ObjectHelper.e().a(gVar);
        boolean zB = b();
        if (zB) {
            this.b.setGravity(new Vector2(0.0f, -100.0f));
            this.b.step(0.025f, 3, 3);
            com.censivn.C3DEngine.C3DEngine.e().j();
        }
        com.tsf.shell.manager.app.v.ObjectHelper.e().a(zB);
    }

    public boolean b() {
        return com.tsf.shell.manager.app.v.ObjectHelper.e().a();
    }

    public void a(com.tsf.shell.f.i.PageItem bVar) {
        com.tsf.shell.manager.app.v.ObjectHelper.e().a(bVar);
    }
}
