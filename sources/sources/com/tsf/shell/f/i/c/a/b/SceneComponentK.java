package com.tsf.shell.f.i.c.a.b;

import android.graphics.Bitmap;
import com.tsf.b;

/* JADX INFO: loaded from: C:\Users\Jaja\AndroidStudioProjects\TSF20\resources-Prime\classes.dex */
public class SceneComponentK extends ItemStubHelper {
    private float b = 320.0f;
    private float c = 64.0f;
    private float d = 32.0f;
    private float e = 64.0f;

    @Override // com.tsf.shell.f.i.c.a.b.SceneComponentE
    public /* bridge */ /* synthetic */ void b() {
        super.b();
    }

    public float[] a(int i, int i2) {
        float f = (((this.d * i) + 1.0f) - 5.0f) / this.b;
        float f2 = (((this.d * (i + 1)) - 2.0f) - 5.0f) / this.b;
        float f3 = ((this.e * 0.0f) + 1.0f) / this.c;
        float f4 = ((this.e * 1.0f) - 1.0f) / this.c;
        float f5 = (((this.d * i2) + 2.0f) - (-5.0f)) / this.b;
        float f6 = (((this.d * (i2 + 1)) - 1.0f) - (-5.0f)) / this.b;
        float f7 = ((this.e * 0.0f) + 1.0f) / this.c;
        float f8 = ((this.e * 1.0f) - 1.0f) / this.c;
        return new float[]{f, f3, f, f4, f2, f3, f2, f3, f, f4, f2, f4, f5, f7, f5, f8, f6, f7, f6, f7, f5, f8, f6, f8};
    }

    @Override // com.tsf.shell.f.i.c.a.b.SceneComponentE
    public void a(Bitmap bitmap) {
        this.a = FolderItemGroup.d.createTexture(bitmap, true);
        bitmap.recycle();
    }

    @Override // com.tsf.shell.f.i.c.a.b.SceneComponentE
    public Bitmap c() {
                return com.tsf.shell.f.i.c.a.NodeConstants.a(FolderItemGroup.c, DeferredItemGroup.d.widget_preview_cubeclock_number);
    }
}
