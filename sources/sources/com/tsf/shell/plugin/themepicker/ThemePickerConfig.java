package com.tsf.shell.plugin.themepicker;

import android.content.Context;
import android.content.pm.PackageManager;
import android.content.pm.ResolveInfo;
import android.os.Bundle;
import android.support.v4.app.Fragment;
import android.support.v4.app.o;
import android.support.v4.app.r;
import com.tsf.shell.plugin.themepicker.ThemePickerResources;
import java.util.List;

/* JADX INFO: loaded from: C:\Users\Jaja\AndroidStudioProjects\TSF20\resources-Prime\classes.dex */
public class ThemePickerConfig extends ThemePickerResult {
    private List<ResolveInfo> a;
    private PackageManager b;
    private Context c;

    public ThemePickerConfig(o oVar, List<ResolveInfo> list, Context context) {
        super(oVar);
        this.c = context;
        this.b = context.getPackageManager();
        this.a = list;
    }

    @Override // android.support.v4.app.r
    public Fragment c() {
        if (i == 0) {
            return new ThemePickerProvider();
        }
        i iVar = new ThemePickerManager();
        Bundle bundle = new Bundle();
        bundle.putInt("PageIndex", i - 1);
        bundle.putParcelable("resolveInfo", this.a.get(i - 1));
        iVar.g(bundle);
        return iVar;
    }

    @Override // android.support.v4.view.ac
    public int b() {
        if (this.a != null) {
            return 1 + this.a.size();
        }
        return 1;
    }

    @Override // android.support.v4.view.ac
    public CharSequence c() {
        if (i == 0) {
            return this.c.getString(ThemePickerResources.j.theme_picker_theme_picker_gallery_apps);
        }
        String str = "";
        for (String str2 : this.a.get(i - 1).loadLabel(this.b).toString().split(" ")) {
            if (str.length() < 10) {
                str = str + str2 + " ";
            }
        }
        if (str.length() > 11) {
            str.subSequence(0, 10);
            return str;
        }
        return str;
    }
}
