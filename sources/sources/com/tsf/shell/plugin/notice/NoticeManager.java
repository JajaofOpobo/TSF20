...
import com.tsf.shell.plugin.notice.NoticeResources;
...
public class NoticeManager {
    public static void a(Context context, NodeList nodeList, NoticeStateProvider.a aVar, boolean z) {
        ...
        for (int i = 0; i < length; i++) {
            try {
                ...
                NoticeEvent.c("versionCode:" + iIntValue);
                ...
                if (packageInfo.versionCode >= iIntValue) {
                    NoticeEvent.c("break by version code:" + attribute2);
                } else {
                    if (a(context, attribute2 + attribute, Integer.valueOf(element.getAttribute("max")).intValue())) {
                        if (Boolean.valueOf(element.getAttribute("beta")).booleanValue() && !z) {
                            NoticeEvent.c("break by version beta:" + attribute2);
                        } else {
                            ...
                            builder.setSmallIcon(NoticeResources.a.ic_update).setContentIntent(activity).setContentTitle(str).setLargeIcon(bitmapCreateScaledBitmap).setWhen(System.currentTimeMillis()).setContentText(context.getString(NoticeResources.b.update_notic_update_available)).setAutoCancel(true);
