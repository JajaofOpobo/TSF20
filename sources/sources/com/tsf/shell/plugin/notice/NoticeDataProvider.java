        ...
        for (int i = 0; i < length; i++) {
            try {
                ...
                if (!attribute.equals("") && a(context, attribute, Integer.valueOf(element.getAttribute("max")).intValue())) {
                    ...
                    if (!NoticeStateProvider.a(packageManager, attribute2)) {
                        Intent intent = new Intent("android.intent.action.VIEW");
                        intent.setData(Uri.parse(element.getAttribute("intent")));
                        PendingIntent activity = PendingIntent.getActivity(context, 0, intent, 134217728);
                        byte[] bArrA = NoticeRenderer.a(element.getAttribute("iconUrl"));
                        ...
                        builder.setSmallIcon(NoticeResources.a.ic_update).setContentIntent(activity).setContentTitle(attribute3).setLargeIcon(bitmapCreateScaledBitmap).setWhen(System.currentTimeMillis()).setContentText(attribute4).setAutoCancel(true);
                    ...
                    NoticeEvent.c("Notify App recommend : " + attribute2);
                }
            } catch (Exception e) {
