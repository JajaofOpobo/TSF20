...
import com.tsf.shell.utils.ExternalFilesDirResolver;
...
public class BackupExportManager {
        ...
        if (fileD.exists()) {
            ...
            for (File file : fileArrListFiles) {
                if (!file.isDirectory() && file.getName().endsWith(".config")) {
                    ...
                    arrayList.add(new BackupEntry(file.getName().substring(0, r4.length() - 7), simpleDateFormat.format(calendar.getTime()), file.getParent() + "/"));
                }
            }
        }
        new BackupRestoreManager().a(context, arrayList, context.getString(b.i.mn_title_db_import));
    }
        ...
        if (file.exists()) {
            ...
            for (File file2 : file.listFiles()) {
                if (!file2.isDirectory() && file2.getName().endsWith(".db")) {
                    ...
                    arrayList.add(new BackupEntry(file2.getName().substring(0, r5.length() - 3), simpleDateFormat.format(calendar.getTime()), file2.getParent() + "/"));
                }
            }
            for (BackupEntry backupEntry : arrayList) {
        ...
        if (z) {
            builder.setPositiveButton(b.i.public_action_ok, new DialogInterface.OnClickListener() { // from class: com.tsf.shell.manager.b.d.1
                ...
                public void onClick(DialogInterface dialogInterface, int i) {
                    BackupExportManager.b(context, editText.getText().toString(), runnable);
                }
            });
        }
        if (z2) {
    ...
    public static void b(Context context, String str, Runnable runnable) {
        new ExportTask(context, str, runnable).execute(new Void[0]);
    }

    static class ExportTask extends AsyncTask<Void, Void, String> {
        ...
        public String doInBackground(Void... voidArr) {
            ...
            BackupExportManager.d();
            return BackupExportManager.b(this.c, this.d, this.b);
        }
