...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
        ...
        switch (action) {
            ...
            case 2:
                ...
                if (p()) {
                    ...
                    if (fAbs > this.a && (!this.o || fAbs > Math.abs(f3))) {
    ...
    protected final void onRestoreInstanceState(Parcelable parcelable) {
        if (parcelable instanceof Bundle) {
            ...
            j jVarA = j.a(bundle.getInt("ptr_state", 0));
            if (jVarA == j.REFRESHING || jVarA == j.MANUAL_REFRESHING) {
                a(jVarA, true);
            }
            a(bundle);
            return;
    ...
    private void q() {
        ...
        switch (this.k) {
            case PULL_FROM_END:
                iRound = Math.round(Math.max(f2 - f3, 0.0f) / 2.0f);
                footerSize = getFooterSize();
                break;
            default:
                iRound = Math.round(Math.min(f2 - f3, 0.0f) / 2.0f);
        ...
        if (iRound != 0 && !i()) {
            ...
            if (this.i != j.PULL_TO_REFRESH && footerSize >= Math.abs(iRound)) {
                a(j.PULL_TO_REFRESH, new boolean[0]);
            } else if (this.i == j.PULL_TO_REFRESH && footerSize < Math.abs(iRound)) {
    ...
    final class i implements Runnable {
        ...
        public void run() {
            if (this.h == -1) {
                ...
                this.i = this.d - Math.round(this.b.getInterpolation(Math.max(Math.min(((System.currentTimeMillis() - this.h) * 1000) / this.e, 1000L), 0L) / 1000.0f) * (this.d - this.c));
