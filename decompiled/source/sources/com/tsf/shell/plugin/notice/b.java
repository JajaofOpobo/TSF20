                ...
                if (packageInfo.versionCode >= intValue) {
                    ...
                    if (a(context, attribute2 + attribute, Integer.valueOf(element.getAttribute("max")).intValue())) {
                        if (Boolean.valueOf(element.getAttribute("beta")).booleanValue() && !z) {
                            ...
                            builder.setSmallIcon(f.a.ic_update).setContentIntent(activity).setContentTitle(str).setLargeIcon(createScaledBitmap).setWhen(System.currentTimeMillis()).setContentText(context.getString(f.b.update_notic_update_available)).setAutoCancel(true);
